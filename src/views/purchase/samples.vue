    ...
    <el-card>
      <div slot="header" class="clearfix">
        ...
        <div style="float:right">
          ...
          <el-button type="primary" icon="el-icon-plus" size="small" @click="openCreate">新增送样</el-button>
        </div>
      </div>
      ...
      <el-dialog title="送样详情" :visible.sync="detailVisible" width="880px">
        <div v-if="current">
          <p><strong>送样单号：</strong>{{ current.sampleNo }} &nbsp;&nbsp; <strong>供应商：</strong>{{ current.supplier }}</p>
          <p><strong>联系人：</strong>{{ current.contactName || '-' }} / {{ current.contactPhone || '-' }}</p>
          <p><strong>发货日期：</strong>{{ current.shipDate || '-' }} &nbsp;&nbsp; <strong>到货日期：</strong>{{ current.deliveryDate || '-' }}</p>
          <p><strong>物流：</strong>{{ current.expressCompany || '-' }} / {{ current.trackingNumber || '-' }}</p>
          <p><strong>状态：</strong>{{ current.status || '-' }} &nbsp;&nbsp; <strong>备注：</strong>{{ current.remark || '-' }}</p>
      ...
      <el-dialog :title="isEdit ? '编辑送样' : '新增送样'" :visible.sync="editVisible" width="980px">
...
export default {
  ...
  methods: {
    ...
    emptyItem() {
      return { materialCode: '', materialName: '', specification: '', quantity: 1, unit: '', remark: '' }
    },
    statusTag(status) {
      if (status === '已签收') return 'success'
      if (status === '运输中') return 'info'
      return 'warning'
    },
    async fetchList() {
      this.loading = true
      try {
        const res = await listPurchaseSamples({ page: this.pagination.page, size: this.pagination.size, supplier: this.filters.supplier, status: this.filters.status })
    ...
    async openDetail(row) {
      const res = await getPurchaseSampleDetail(row.sampleNo)
      if (res && (res.code === 200 || res.code === 20000)) {
    ...
    async save() {
      ...
      const api = this.isEdit ? updatePurchaseSample : createPurchaseSample
    ...
    confirmDelete(row) {
      this.$confirm(`确认删除送样单【${row.sampleNo}】?`, '提示', { type: 'warning' })
        .then(() => this.remove(row.sampleNo))
        .catch(() => {})
    },
    async remove(sampleNo) {
      const res = await deletePurchaseSample(sampleNo)
      if (res && (res.code === 200 || res.code === 20000)) {
