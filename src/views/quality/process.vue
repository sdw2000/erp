  ...
  <div class="app-container">
    <el-card shadow="never">
      ...
      <el-table v-loading="loading" :data="rows" border stripe>
        ...
        <el-table-column prop="processNode" label="工序节点" width="120" />
    ...
    <el-dialog title="过程检测详情" :visible.sync="detailVisible" width="600px">
    ...
    <el-dialog title="新增过程检测" :visible.sync="createVisible" width="520px" @close="resetForm">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="工序节点" prop="processNode">
          <el-input v-model="form.processNode" placeholder="如 coating/rewinding/slitting" />
        </el-form-item>
...
<script>
import { listProcessInspections, deleteProcessInspection, createProcessInspection, getDefectTypeList } from '@/api/quality'
...
export default {
  ...
  data() {
    return {
      ...
      defectTypes: [],
      rules: {
        processNode: [{ required: true, message: '请输入工序节点', trigger: 'blur' }],
        batchNo: [{ required: true, message: '请输入批次号', trigger: 'blur' }],
        rollCode: [{ required: true, message: '请输入卷码', trigger: 'blur' }],
  ...
  methods: {
    ...
    submitForm() {
      this.$refs.form.validate(async valid => {
        if (!valid) return
        const res = await createProcessInspection(this.form)
        if (res && (res.code === 200 || res.code === 20000)) {
    ...
    resetForm() {
      ...
      this.form = { processNode: '', batchNo: '', rollCode: '', materialCode: '', sampleQty: 0, failQty: 0, overallResult: 'pending', inspectorName: '', defectType: '', remark: '' }
    },
