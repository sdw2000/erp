<template>
  <div class="app-container">
    ...
    <el-dialog title="出货检测详情" :visible.sync="detailVisible" width="600px">
    ...
    <el-dialog title="新增出货检测" :visible.sync="createVisible" width="520px" @close="resetForm">
      <el-form ref="form" :model="form" :rules="rules" label-width="100px">
        <el-form-item label="出货单号" prop="sourceOrderNo">
          <el-input v-model="form.sourceOrderNo" />
        </el-form-item>
...
<script>
import { listOutboundInspections, deleteOutboundInspection, createOutboundInspection, getDefectTypeList } from '@/api/quality'
...
export default {
  ...
  data() {
    return {
      ...
      defectTypes: [],
      rules: {
        sourceOrderNo: [{ required: true, message: '请输入出货单号', trigger: 'blur' }],
  ...
  methods: {
    ...
    submitForm() {
      this.$refs.form.validate(async valid => {
        if (!valid) return
        const res = await createOutboundInspection(this.form)
        if (res && (res.code === 200 || res.code === 20000)) {
    ...
    resetForm() {
      ...
      this.form = { sourceOrderNo: '', batchNo: '', rollCode: '', materialCode: '', sampleQty: 0, passQty: 0, failQty: 0, overallResult: 'pending', inspectorName: '', defectType: '', remark: '' }
    },
